...
st.set_page_config(page_title="台股AI標股神探 (精準修正版)", layout="wide")

# --- 1. 定義百大熱門股資料庫 (已人工校對 .TW 與 .TWO) ---
# 格式：(完整代號, 名稱) -> 這裡直接寫死正確的後綴
INIT_STOCKS = [
    # === 半導體與 AI (上市) ===
    ("2330.TW", "台積電"), ("2454.TW", "聯發科"), ("2317.TW", "鴻海"), ("2303.TW", "聯電"), ("3711.TW", "日月光"),
    ...
    ("3017.TW", "奇鋐"), ("3324.TW", "雙鴻"), ("2301.TW", "光寶科"), ("2368.TW", "金像電"), ("3037.TW", "欣興"),
    ("3044.TW", "健鼎"), ("2313.TW", "華通"), ("2383.TW", "台光電"), ("2449.TW", "京元電子"), ("6274.TW", "台燿"),
    ...
    ("5880.TW", "合庫金"), ("2887.TW", "台新金"), ("2834.TW", "臺企銀"), ("2801.TW", "彰銀"), ("2888.TW", "新光金"),
    ("5876.TW", "上海商銀"), ("2812.TW", "台中銀"), ("5871.TW", "中租-KY"),
...
def validate_and_search(query):
    ...
    # 優先查字典 (這裡現在非常準確，因為包含了 .TWO 的正確對應)
    if query in tw_stock_dict:
        # 如果是輸入名稱 (如 "信驊")，字典直接給出 "5274.TWO"
        # 如果是輸入代號 (如 "5274")，字典也已經補上了對應
        full_code = tw_stock_dict[query]
        # 取得名稱 (如果 key 是代號，就要反查名稱；如果 key 是名稱，就是 query)
        name = query if not query.replace('.','').isdigit() else st.session_state.watch_list.get(full_code, f"自選股-{query}")
        return full_code, name, None

    # 反向查找 (Input: 5274)
    # 上面的字典已經處理了大部分，這裡作為保險
        ...
        try:
            ...
            if len(closes_list) < 1:
                ...
                rows.append({
                    ...
                    "reason": "⚠️ API 暫無數據 (可能是代號後綴錯誤)", "trend": []
                })
                continue
...
with st.container():
    ...
    with col_info:
        st.info("💡 **資料庫修復**：已更正「信驊」、「力旺」、「群聯」、「美債20年」等上櫃股票的連線設定，現在能正常顯示報價了！")
