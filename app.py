            ...
            with c2: 
                if st.form_submit_button("加入自選") and query:
                    ...
                    for q in queries:
                        ...
                        if s: 
                            st.session_state.custom_list[s] = n # 加入自選清單
                            st.session_state.watch_list[s] = n  # 同步總清單(防當機)
                            st.session_state.last_added = s
                            has_new = True
                            st.success(f"✅ 成功將 {n} 加入自選！")
                        else: 
                            st.error(f"❌ {q}：{e}")
                    if has_new:
                        st.rerun()
