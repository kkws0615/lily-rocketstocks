...
# # --- 策略參數常數 (原本魔法數字，改為具名常數方便調整) ---
# VOL_SURGE_THRESHOLD = 1.2   # 爆量門檻：成交量 > 5日均量的幾倍
# BIAS_STRONG_PCT     = 5.0   # 強力推薦所需的月線乖離率 (%)
# NEAR_MA_RANGE       = 0.05  # 「接近年線」的價格容忍範圍 (5%)
# SHORT_TARGET_MULT   = 1.10  # 短線目標價倍數
# MEDIUM_TARGET_MULT  = 1.15  # 中線目標價倍數
# LONG_TARGET_MULT    = 1.30  # 長線目標價倍數
...
# # [修復 Bug #3] RSI 計算避免除以零
# # 原本：直接 gain / loss，當 loss=0 時會產生 NaN 或 inf
# # 修正：用 np.where 處理 loss=0 的情況（股票連漲14天 → RSI 直接給 100）
# def calculate_rsi(series, period=14):
#     delta = series.diff()
#     gain = (delta.where(delta > 0, 0)).rolling(window=period).mean()
#     loss = (-delta.where(delta < 0, 0)).rolling(window=period).mean()
...
# # [修復 Bug #1] 策略 key 統一改為 "long"（原本 t3 tab 傳入 "year"，但此處用 else 接，邏輯不一致）
# def analyze_logic(cur_p, ma20, ma60, ma120, ma240, vol_ratio, rsi, strategy="short"):
#     if ma60 is None:
#         return "觀察", "tag-hold", 40, "資料不足", cur_p
#     bias_20 = ((cur_p - ma20) / ma20) * 100
...
# # [修復 Bug #2] fetch_data 改成接受 tuple（list 不可 hash，@st.cache_data 無法正確快取）
# @st.cache_data(ttl=300)
# def fetch_data(tickers: tuple):
#     if not tickers:
#         return None
...
#     # [修復 Bug #2] 傳入 tuple 以確保快取正常運作
#     data = fetch_data(tuple(sorted(tickers)))
#     rows = []
#     for t in tickers:
#         try:
#             # [修復 Bug #2] 單一股票時 yf.download 回傳平坦 DataFrame（無 MultiIndex）
#             # 原本 else: df = data 是把整個 data 物件直接賦值，會導致錯誤
#             # 修正：單一股票時直接用 data 本身（它就是那支股票的 DataFrame）
#             if isinstance(data.columns, pd.MultiIndex):
#                 if t not in data.columns.get_level_values(0):
#                     continue
#                 df = data[t]
#             else:
#                 # 只有一支股票，data 本身就是該股票的 DataFrame
#                 df = data
...
#         # [修復 Warning #3] min==max 時改畫平線，而非直接 skip 整列
#         if mx == mn:
#             spark = '<svg width="150" height="40"><line x1="0" y1="20" x2="150" y2="20" stroke="#aaa" stroke-width="2"/></svg>'
#         else:
#             pts = " ".join([f"{(i / (len(r['trend']) - 1)) * 150},{40 - ((v - mn) / (mx - mn)) * 30 - 5}" for i, v in enumerate(r['trend'])])
...
#     # [修復 Warning #4] 移除脆弱的 st.write("") 對齊法，改用 st.container 包裝
#     with col_btn:
#         with st.container():
#             if st.button("🔄 刷新大盤熱門股", help="更新前三個 Tab 的百大熱門名單", use_container_width=True):
#                 fetch_dynamic_hot_stocks.clear()
...
#     components.html(render_table(rows, d2), height=600, scrolling=True)
# with t3:
#     # [修復 Bug #1] 原本傳 "year"，但 analyze_logic 用 else 接，語意不清
#     # 統一改為 "long"，與註解和邏輯一致
#     rows = process_display(st.session_state.watch_list, "long")
#     components.html(render_table(rows, d3), height=600, scrolling=True)
# with t4:
